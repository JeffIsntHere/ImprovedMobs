...
public class EntityModifyFlagConfig implements IConfigListValue<EntityModifyFlagConfig> {
    ...
    public void resolveTags() {
        ...
        for (String val : this.tagsEntryToResolve) {
            ...
            Iterable<Holder<EntityType<?>>> tag = Registry.ENTITY_TYPE.getTagOrEmpty(TagKey.create(Registry.ENTITY_TYPE_REGISTRY, new ResourceLocation(subs[0].trim())));
            tag.forEach(h -> this.map.put(Registry.ENTITY_TYPE.getKey(h.value()).toString(), set));
        }
    }
